...
var sigset_none = sigset(uint64(0))
...
func minit() {
	_g_ := getg()
	asmcgocall(unsafe.Pointer(funcPC(miniterrno)), unsafe.Pointer(&libc__errnop))
...
func pthread_attr_getstacksize(attr *pthreadattr, size *uint64) int32 {
	return int32(sysvicall2(&libc_pthread_attr_getstacksize, uintptr(unsafe.Pointer(attr)), uintptr(unsafe.Pointer(size))))
}
